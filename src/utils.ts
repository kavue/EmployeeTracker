...
export const validateSalary = (input: string) => {
...
export const displayTable = (rows: any[], columns: string[]) => {
...
export const getDepartmentChoices = async () => {
  ...
  return departments.rows.map((dept: any) => ({ name: dept.name, value: dept.id }));
};

export const getRoleChoices = async () => {
  const roles = await pool.query('SELECT id, title FROM role');
  return roles.rows.map((role: any) => ({ name: role.title, value: role.id }));
};

export const getManagerChoices = async () => {
  const employees = await pool.query('SELECT id, first_name, last_name FROM employee');
  return employees.rows.map((emp: any) => ({ name: `${emp.first_name} ${emp.last_name}`, value: emp.id }));
};
